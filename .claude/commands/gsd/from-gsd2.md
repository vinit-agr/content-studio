...
1. **Locate the .gsd/ directory** — check the current working directory (or `--path` argument):
   ```bash
   node "/Users/vinit/Tars/Content-Creation/content-studio/.claude/get-shit-done/bin/gsd-tools.cjs" from-gsd2 --dry-run
   ```
...
3. **Run the migration** after confirmation:
   ```bash
   node "/Users/vinit/Tars/Content-Creation/content-studio/.claude/get-shit-done/bin/gsd-tools.cjs" from-gsd2
   ```
